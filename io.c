...
static _st_netfd_t *_st_netfd_new(int osfd, int nonblock, int is_socket) {
    ...
    if (nonblock) {
        ...
        if (is_socket == FALSE)
            return (fd);
        ioctlsocket(fd->osfd, FIONBIO, (u_long *) &flags);
#else
...
_st_netfd_t *st_accept(_st_netfd_t *fd, struct sockaddr *addr, int *addrlen,
    ...
    while ((osfd = accept(fd->osfd, addr, (socklen_t *) addrlen)) < 0) {
        if (osfd != INVALID_SOCKET) {
            //if(addr!=NULL) printf("accept：%s \r\n", addr->sa_data);
            break;
        }
        errno = _st_GetError(0);
...
int readv(int sock, struct iovec *iov, int nvecs) {
    DWORD ret;
...
ssize_t st_readv(_st_netfd_t *fd, const struct iovec *iov, int iov_size,
    ...
    while ((n = readv(fd->osfd, (struct iovec *) iov, iov_size)) < 0) {
        if (errno == EINTR)
            continue;
        if (!_IO_NOT_READY_ERROR)
            return -1;
...
int st_writev_resid(_st_netfd_t *fd, struct iovec **iov, int *iov_size,
    ...
    while (*iov_size > 0) {
        if (*iov_size == 1)
#ifdef WIN32
            n = send(fd->osfd, (*iov)->iov_base, (*iov)->iov_len, 0);
#else
...
#ifdef WIN32
    while ((osfd = open(path, oflags | O_BINARY, mode)) < 0) {
        errno = _st_GetError(0);
#else
